...
class FileAnalyzer:
    ...
    def analyze_files(self) -> Optional[ Dict ]:
        ...
        for pyF in pyFiles:
            ...
            self.files_summary = { pyF_Name: {'Classes': pyF_ClassNum, 'Functions': pyF_FuncNum, 'Lines': pyF_LineNum, 'Characters': pyF_CharNum} }
            self.all_files_summary.append(self.files_summary)

    # ((/))

    # ((method))
    def pretty_print(self) -> None:
        """ Print out the formatted presentation of the given analyzed summary info from the Python files. """
                  
        for file_name, data_pairs in self.files_summary.items():
            data_pairs: Dict
            pairs_keys = [ pairKey for pairKey, pairData in data_pairs.items() ]

        table = PrettyTable( ['File Name'] + pairs_keys )

        unit: Dict
        for unit in self.all_files_summary:            
            value: Dict
            for key, value in unit.items():                
                data_fields = [data for data_key, data in value.items()]                        
            
                curDir = os.getcwd()
                full_path: str = os.path.join(curDir, key)
                
                table.add_row([full_path] + data_fields)
...
def main(): #!! <- Main Program Procedure Flow !!#
    ...
    # // "run # 02 function"
    print (f"v The function # 02's performing is below: v")
    SL()
    ...
    # // "run # 03 class"
    print (f"v The function # 03's performing is below: v")
    FileAnalyzer("HW08_Attached-Files/").pretty_print()
    #osm.analyze_files()
    SL()
    BL()
