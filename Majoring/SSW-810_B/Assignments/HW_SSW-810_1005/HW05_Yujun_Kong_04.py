...
def get_lines(target_FileName_withPath: str) -> Iterator[str]:
    ...
    try:
        ...
        with target_file:
            ...
            commentsChar_Killed_Lines: list = []
            for single_Line in all_Lines:
                ...
                comments_Char_Index: int = single_Line.find('#')

                if (lax == '\\'):
                    #single_Line.rsplit()
                    single_Line = single_Line[0 :comments_Char_Index]
                    ...
                    #single_Line.rsplit()
                    commentsChar_Killed_Lines.append(single_Line)

            ckl = ''.join(commentsChar_Killed_Lines)
            #print (ckl)

            for standard_Line in ckl:
                comm_Char_Idx: int = standard_Line.find('#')
                leftArrow_Idx: int = standard_Line.find('<')

                if '#' in standard_Line:
                    continue
                    standard_Line = standard_Line[:comm_Char_Idx]

            print (ckl)

                #standard_Line[comm_Char_Idx:leftArrow_Idx]
