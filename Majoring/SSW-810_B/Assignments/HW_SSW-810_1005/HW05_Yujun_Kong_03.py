...
def get_lines(target_FileName_withPath: str) -> Iterator[str]:
    ...
    try:
        ...
        with target_file:
            ...
            for line_By_Line in commentsChar_Killed_Lines:
                ckl = ''.join(line_By_Line)
                print (ckl)

            standard_Lines: list = []
            for ck_Line in ckl:

                first_IndexOf_SingleLine: int = ck_Line[0]
                fix = first_IndexOf_SingleLine

                last_IndexOf_SingleLine: int = len(ck_Line) - 1
                lax = last_IndexOf_SingleLine

                if (fix == '<') and (lax == '>'):
                    standard_Lines.append(ck_Line)
