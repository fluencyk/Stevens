...
class Fraction:
    ...
    def __init__(self, nutor: int, detor: int) -> None:
        ...
        if self.validate(nutor, detor) is True:
            self.simplify()

    def validate(self, nur: int, der: int) -> bool:
        """ validate if deniminator zero, process negative notation and zero value """
        if nur == 0:
            self.value = "0"
        if der == 0:
            self.value = "Not a Valid Fraction!"
            return False
        if nur < 0 and der < 0:
...
class FractionsCalculator(Fraction):
    ...
    def devide(self, f_1: "Fraction", f_2: "Fraction") -> None:
        ...
        elif f_1.value != "0" and f_2.value != "0":
...
def main():
    ...
    f_a = Fraction(1, -2)
    print(f"Fraction A is '{f_a.ogl_value}'")
    f_b = Fraction(1, 2)
    print(f"Fraction B is '{f_b.ogl_value}'")
