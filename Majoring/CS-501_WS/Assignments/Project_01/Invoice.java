...
public class Invoice<E> {
	...
	private void setMsg() {
		...
		msgCancel = "User cancelled, invoice program quits.";
	...
	private boolean isTipCorrect() {
		...
		for (int i = 0; i < this.input.length(); i++) {
			if (!allNums.contains(toString(this.input.charAt(i)))) {
				return false;
			} else if (this.input.startsWith("0") || this.input.startsWith("-") || this.input.startsWith(".") || this.input.endsWith("-") || this.input.endsWith(".")) {
				return false;
			} else {
